...
export default class opportunity3 extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            textOptionArr: [
                ...
                "6)	คู่แข่งขัน มีข้อด้อยคือร้านค้าเก่า ทรุดโทรม ขาดความน่าเชื่อถือและความ",
    ...
    handleClick = async (button) => {
        ...
        let totalScore = 0.000                                                      // Total Score.
            ...
            //  console.log(this.state.s_option_arr)
             ...
             for (var i = 0; i < checkedArr.length; i++) {
                ...
                for (var j = 0; j < this.state.s_option_arr.length; j++) { 
                    ...
                    if (tempCheck === `s-option${j + 1}`) {                         // Condition to find option that checked.
                        await this.state.optionInfoArr.push({                       // If found save optionText & Score to state.
        ...
        // If checkbox is unchecked.                                                // Same for above function (checked -> unchecked).
            ...
            await this.setState({ alreadyChecked_arr: checkedArr.filter((n) => !tempUnCheckedArr.includes(n)) })
    ...
    handleSubmit = () => {
        ...
        for (var i = 0; i < optionInfoArr.length; i++) {
            optionInfoArr[i].optionScore = parseFloat(optionInfoArr[i].optionScore / totalScore).toFixed(3)
        }
    ...
    render() {
        ...
        return (
            <div className="wrapper-swot-form weakness" style={{ padding: "60px", borderRadius: "20px", color: this.state.color  }}>
                ...
                <div id="buttonNextCenter"><button id="buttonNext" onClick={this.handleSubmit}>ถัดไป</button></div>
            </div>
    );
  }
}
