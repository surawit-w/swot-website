...
handleSubmit = () => {
    ...
    if(parseFloat(this.state.totalScore) === parseFloat(1.00)) {
        ...
        // console.log(true)
        localStorage.setItem('opportunity_result', JSON.stringify(this.state))
        let details = JSON.parse(localStorage.getItem('opportunity_result'))
        console.log(details.alreadyCheckText_arr) // ได้ค่ามาละ 5555 
        
        
        document.location.href = '/swot-analysis-threat'
    }
}

  render() {
    return (
          <div className="wrapper-swot-form opportunity" style={{  padding: '60px', borderRadius: '20px'}}>
