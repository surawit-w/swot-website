...
handleSubmit = () => {
    ...
    if(parseFloat(this.state.totalScore) === parseFloat(1.00)) {
        ...
        // console.log(true)
        localStorage.setItem('weakness_result', JSON.stringify(this.state))
        let details = JSON.parse(localStorage.getItem('weakness_result'))
        console.log(details.alreadyCheckText_arr) // ได้ค่ามาละ 5555 
        
        
        document.location.href = '/swot-analysis-opportunity'
    }
}

  render() {
    return (
          <div className="wrapper-swot-form weakness" style={{  padding: '60px', borderRadius: '20px'}}>
