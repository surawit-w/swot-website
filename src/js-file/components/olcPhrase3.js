...
export default class olcPhrase3 extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            alreadyChecked_arr: [],
            s_option_arr: ["s-option1","s-option2","s-option3","s-option4","s-option5","s-option6","s-option7","s-option8","s-option9","s-option10"],
            textOptionArr: ['กิจการเติบโตอย่างเต็มที่ มีฐานลูกค้าค่อนข้างมาก ทำให้สินค้า หรือวัตถุดิบที่ใช้จำหน่ายไม่เพียงพอต่อลูกค้าในบางครั้ง เช่น ร้านขายอาหารมีการหุงข้าวไม่เพียงพอต่อจำนวนลูกค้าที่เข้าใช้บริการ หรือร้านสะดวกซื้อ สินค้าบางรายการอาจจะไม่พอต่อความต้องการลูกค้า เนื่องจากจำนวนสินค้าที่สต็อกไม่เพียงพอ',
            ...
            option_arr: [],
            score_arr: [0.19, 0.16, 0.14, 0.09, 0.08, 0.07, 0.05, 0.05, 0.07, 0.10],
            left_option_arr: [],
        ...
        // If checkbox is unchecked.
            ...
            for(i = 0; i < this.state.alreadyChecked_arr.length; i++) {
                for(j = 0; j < this.state.s_option_arr.length; j++) {
                    ...
                    if(tempUnCheckedArr[0] === `s-option${j+1}`) { totalScore = this.state.totalScore - this.state.score_arr[j] }
                    if(tempCheck === `s-option${j+1}`) {
                        await this.state.optionInfoArr.push({
                            optionText: this.state.textOptionArr[j],
                            optionScore: this.state.score_arr[j]
    ...
    renderOLCForm = (e, i) => (
        <div className="option-checkbox">
            <label class="check">
            <input type="checkbox" id={`s-option${i+1}`} onClick={(event) => this.handleClick(event)}/>
            <div class="box"/>
            </label>
            <p>{e}</p>
        </div>
            ...
            <div className="wrapper-swot-form strength" style={{  padding: '60px', borderRadius: '20px'}}>
                ...
                <div className="form-wrapper" style={{ padding: '16px 20px'}}>
                    <Form style={{ fontFamily: 'light'}}>
                        <fieldset>
                            <Form.Group as={Row}>
                                <Form.Label as="legend" style={{fontSize: '16px', fontFamily: 'regularItalic'}}>
                                    เลือกข้อที่ตรงกับธุรกิจของคุณมากที่สุด (สามารถเลือกได้มากกว่า 1 ข้อ)
                                </Form.Label>
                ...
                <button onClick={this.handleSubmit}>ถัดไป</button>
            </div>
        );
    }
}
