...
export default class olcPage extends Component {
	...
	OLCPhrase1Callback = async (dataFromChild, totalScore) => {
		...
		await this.state.allTotalScore.push({phraseScore: 'OLC1', totalScore: parseFloat(totalScore)})
	}
	...
	OLCPhrase2Callback = async (dataFromChild, totalScore) => {
		...
		await this.state.result_arr.push(dataFromChild)
		await this.state.allTotalScore.push({phraseScore: 'OLC2', totalScore: parseFloat(totalScore)})
	}
  	...
  	OLCPhrase3Callback = async (dataFromChild, totalScore) => {
		...
		await this.state.result_arr.push(dataFromChild)
		await this.state.allTotalScore.push({phraseScore: 'OLC3', totalScore: parseFloat(totalScore)})
	}
	...
	OLCPhrase4Callback = async (dataFromChild, totalScore) => {
		...
		await this.state.result_arr.push(dataFromChild)
		await this.state.allTotalScore.push({phraseScore: 'OLC4', totalScore: parseFloat(totalScore)})
	}
	...
	OLCPhrase5Callback = async (dataFromChild, totalScore) => {
		...
		await this.state.allTotalScore.push({phraseScore: 'OLC5', totalScore: parseFloat(totalScore)})
		await this.state.allTotalScore.sort(function(a,b){return Math.abs(b.totalScore) - Math.abs(a.totalScore)})
		...
		for(let i = 0; i < this.state.allTotalScore.length; i++) {
			if(this.state.allTotalScore[0].totalScore === this.state.allTotalScore[i].totalScore) {
				console.log(true)
				console.log(this.state.allTotalScore[0].phraseScore, this.state.allTotalScore[i].phraseScore)
				await this.state.totalResult.push({phraseScore: this.state.allTotalScore[i].phraseScore, totalScore: this.state.allTotalScore[i].totalScore })
				console.log(this.state.totalResult)
			}
		}
		for(let i = 0; i < olcScoreData.length; i++) {
			...
			for(let j = 0; j < 5; j++) {
				...
				olcScoreData[i].sort(function(a,b){return Math.abs(b.optionScore) - Math.abs(a.optionScore)})
		...
		for(let i = 0; i < this.state.totalResult.length; i++) {
			switch(this.state.totalResult[i].phraseScore) {
                case "OLC1":
					this.state.optionResult.push({phraseScore: "OLC1", option: olcScoreData[0]})
                    break;
                case "OLC2":
					this.state.optionResult.push({phraseScore: "OLC2", option: olcScoreData[1]})
                    break;
                case "OLC3":
					this.state.optionResult.push({phraseScore: "OLC3", option: olcScoreData[2]})
                    break;
                case "OLC4":
					this.state.optionResult.push({phraseScore: "OLC4", option: olcScoreData[3]})
					break;
				case "OLC5":
					this.state.optionResult.push({phraseScore: "OLC5", option: olcScoreData[4]})
					break;
                default:
                    break;
            }
  	...
  	// Render elements
  	render() {
		let { isPhrase1, isPhrase2, isPhrase3, isPhrase4, isPhrase5 } = this.state
		return (
		<div>
			<Navbar/>
        	...
        	<div className="column-divider">
				{isPhrase1 && <Phrase1 callbackFromParent={this.OLCPhrase1Callback}/>}
				{isPhrase2 && <Phrase2 callbackFromParent={this.OLCPhrase2Callback}/>}
				{isPhrase3 && <Phrase3 callbackFromParent={this.OLCPhrase3Callback}/>}
				{isPhrase4 &&  <Phrase4 callbackFromParent={this.OLCPhrase4Callback}/>}
				{isPhrase5 &&  <Phrase5 callbackFromParent={this.OLCPhrase5Callback}/>}
			</div>
        </div>
    );
  }
